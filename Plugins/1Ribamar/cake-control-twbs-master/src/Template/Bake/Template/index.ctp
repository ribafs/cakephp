...
<%
    ...
    %>
                <td class="actions">
                    <?= $this->Html->link('<span class="glyphicon glyphicon-zoom-in"></span><span class="sr-only">' . __('Visializar') . '</span>', ['action' => 'view', <%= $pk %>], ['escape' => false, 'class' => 'btn btn-xs btn-default', 'title' => __('Visualizar')]) ?>
                    ...
                    if(isset($loguser)){
                    	if($group !=4){
							print $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span><span class="sr-only">' . __('Excluir') . '</span>', ['action' => 'delete', $controller->id], ['confirm' => __('Excluir realmente o registro # {0}?', $controller->id), 'escape' => false, 'class' => 'btn btn-xs btn-danger', 'title' => __('Excluir')]);
                    	}
                    }
	   	    ?>
                </td>
