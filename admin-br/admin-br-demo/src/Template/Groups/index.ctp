...
<div class="actions columns col-lg-2 col-md-3">
	...
	<li class="active"><?= $this->Html->link(__('Listar Group'), ['action' => 'index']) ?></li>	
    ...
    <div class="table-responsive">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link(__('<span class="glyphicon glyphicon-zoom-in"></span><span class="sr-only">' . __('Ver') . '</span>'), ['action' => 'view', $group->id],['escape' => false,'class' => 'btn btn-xs btn-default', 'title' => __('Ver')]) ?>
                    <?= $this->Html->link(__('<span class="glyphicon glyphicon-pencil"></span><span class="sr-only">' . __('Editar') . '</span>'), ['action' => 'edit', $group->id],['escape' => false, 'class' => 'btn btn-xs btn-default', 'title' => __('Editar')]) ?>
                    <?= $this->Form->postLink(__('<span class="glyphicon glyphicon-trash"></span><span class="sr-only">' . __('Excluir') . '</span>'), ['action' => 'delete', $group->id], ['confirm' => __('Deseja excluir realmente # {0}?', $group->id), 'escape' => false, 'class' => 'btn btn-xs btn-danger', 'title' => __('Excluir')]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
